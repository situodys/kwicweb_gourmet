...
const Register = (props) => {
    ...
    const isAlreadySignup = async () => {
        const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/member/${email}`);
    ...
    const sendEmail = async () => {
        ...
        setIsEmailSent(true);
        const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/email/auth/${email}`);
    }
    ...
    const handleEmailAuthSubmit = async () => {
        const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/email/auth/${email}/code`);
    ...
    const handleRegisterSubmit = async () => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/auth/signup`, toData());
                            ...
                            >
                                <div className="card-body p-5 text-center">
                                    <div className="mb-md-5 mt-md-4 pb-5">
                                        ...
                                        {isEmailSent && <CustomInput type={"text"}
                                                                     placeHolder={"auth code"}
                                                                     handleInput={handleAuthCode}></CustomInput>}

                                        {isEmailSent && <button
                                            className="btn btn-outline-light btn-lg col-4 mb-4"
                                            type="submit"
                                        ...
                                        <CustomInput type={"password"}
                                                     ...
                                                     handleInput={handlePasswordConfirm}></CustomInput>
                                        <div
                                            className={" mb-5"}
                                            style={{
                                            color: isValidPasswordConfirm ? 'green' : 'red',
                                            height: "20px"
                                        }}>{passwordConfirmMsg}</div>

                                        <button
                                            disabled={!(isAuthenticatedEmail && !isDuplicatedEmail && isValidPasswordConfirm && isValidPassword)}
